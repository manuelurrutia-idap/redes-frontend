...
function renderGraph() {
    ...
    const nodes = currentActivities.map(a => ({
        id: a.id,
        label: a.name,
        title: `<strong>${a.name}</strong><br>Duración: ${a.duration}<br>Costo: ${a.cost}`,
        shape: 'box',
...
function highlightGraph(pathNodes = [], pathEdges = [], cpmNodesInfo = null) {
    ...
    // Reset
    nodesDS.forEach(n => {
        let defaultTitle = n.title;
        if(cpmNodesInfo) {
            const a = currentActivities.find(act => act.id === n.id);
            if (a) defaultTitle = `<strong>${a.name}</strong><br>Duración: ${a.duration}<br>Costo: ${a.cost}`;
        }
        nodesDS.update({
            id: n.id, 
    ...
    pathNodes.forEach(nId => {
        let newTitle = nodesDS.get(nId).title;
        if(cpmNodesInfo && cpmNodesInfo[nId]) {
            const info = cpmNodesInfo[nId];
            newTitle += `<hr style="margin: 4px 0; border-color: rgba(150,150,150,0.2);">
            <strong>ES:</strong> ${info.ES} | <strong>EF:</strong> ${info.EF}<br>
            <strong>LS:</strong> ${info.LS} | <strong>LF:</strong> ${info.LF}<br>
            <strong>Holgura (Slack):</strong> ${info.slack}`;
        }
        nodesDS.update({
            id: nId, 
    ...
    if (cpmNodesInfo) {
        nodesDS.forEach(n => {
            if (!pathNodes.includes(n.id) && cpmNodesInfo[n.id]) {
                const info = cpmNodesInfo[n.id];
                const newTitle = n.title + `<hr style="margin: 4px 0; border-color: rgba(150,150,150,0.2);">
                <strong>ES:</strong> ${info.ES} | <strong>EF:</strong> ${info.EF}<br>
                <strong>LS:</strong> ${info.LS} | <strong>LF:</strong> ${info.LF}<br>
                <strong>Holgura (Slack):</strong> ${info.slack}`;
                nodesDS.update({id: n.id, title: newTitle});
            }
        });
    }
            ...
            if(algo === 'floyd-warshall') {
                ...
                resPanel.innerHTML = `
                    <div class="alert info">
                        ...
                        <div>
                            <strong>Cálculo finalizado.</strong> La matriz de distancias se ha calculado (ver consola).
                        </div>
                    </div>
                `;
                console.log(res);
